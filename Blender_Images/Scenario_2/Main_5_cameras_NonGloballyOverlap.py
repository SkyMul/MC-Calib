...
eul_mat_2 = mathutils.Euler((0.0, math.radians(15.0), 0.0), 'XYZ').to_matrix()
mat_loc_2 = mathutils.Matrix.Translation((-1.0,0,0.1))
...
eul_mat_3 = mathutils.Euler((0.0, math.radians(30), 0.0), 'XYZ').to_matrix()
mat_loc_3 = mathutils.Matrix.Translation((-1.9,0,0.525))
...
# Pose of the second camera wrt to the first camera
eul_mat_4 = mathutils.Euler((0.0, math.radians(-15), 0.0), 'XYZ').to_matrix()
mat_loc_4 = mathutils.Matrix.Translation((1.0,0.0,0.1))
cam4_transform = mat_loc_4 * eul_mat_4.to_4x4()

#Generate the fifth camera
cam_obj5 = bpy.data.objects.new("Camera5", cam1)
cam_obj5.location = (9.69, -10.85, 12.388)
cam_obj5.rotation_euler = (0.6799, 0, 0.8254)
scene.objects.link(cam_obj5)
bpy.context.scene.update()
obj_camera5 = bpy.data.objects["Camera5"]
scene.camera = obj_camera5
# Pose of the second camera wrt to the first camera
eul_mat_5 = mathutils.Euler((0.0, math.radians(-30), 0.0), 'XYZ').to_matrix()
mat_loc_5 = mathutils.Matrix.Translation((1.93,0.0,0.525))
cam5_transform = mat_loc_5 * eul_mat_5.to_4x4()

#Write GT of the camera pose
filenameGT = "GroundTruth.yml"
fs = cv2.FileStorage(filenameGT, cv2.FILE_STORAGE_WRITE)
fs.write("nb_camera", 5)
...
K5_blender = get_calibration_matrix_K_from_blender(cam_obj4.data)
K5 = np.identity(3)
K5[0,0] = K5_blender[0][0]
K5[1,1] = K5_blender[1][1]
K5[0,2] = K5_blender[0][2]
K5[1,2] = K5_blender[1][2]
fs.write("K_5", K5)
P5 = np.identity(4)
for i in range(0,4):
    for j in range(0,4):
        P5[i,j] = cam5_transform[i][j]
fs.write("P_5", P5)
